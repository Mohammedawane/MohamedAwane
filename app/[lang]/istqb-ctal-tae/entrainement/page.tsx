...
import CTALTAETraining from "@/app/Component/CTALTAETraining";

export const metadata = {
  title: "Entraînement ISTQB CTAL-TAE | Nexo Skills",
  robots: { index: false, follow: false }, // keep training page private
};

// Cookie that persists paid access for 1 year
const ACCESS_COOKIE = "ctaltae_access";
...
async function verifyPayment(sessionId: string): Promise<boolean> {
  ...
  try {
    ...
    return (
      ...
      session.metadata?.course === "istqb-ctal-tae"
    );
  } catch {
    return false;
  }
...
export default async function EntrainementPage({
  ...
  if (!effectiveId) redirect(`/${lang}/istqb-ctal-tae`);
  ...
  if (!paid) redirect(`/${lang}/istqb-ctal-tae`);
