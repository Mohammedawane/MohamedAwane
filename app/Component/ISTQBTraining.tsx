...
import {
  ...
  type Question, type Domain, type Difficulty, type ExamInfo,
} from "@/app/data/istqb-questions";
...
const LS_KEY = "istqb_v2_progress";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-3xl px-5 py-8 space-y-8">
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-700">
          <strong>Rappel ISTQB FL :</strong> 40 questions · 60 minutes · Seuil de réussite : 65% (26/40)
        </div>
      </div>
    </div>
  );
...
function QuizScreen({
  ...
  const handleSubmit = useCallback(() => {
    if (submitted) return;
    setSubmitted(true);
    const answers: CompletedAnswer[] = questions.map((q, i) => ({
      questionId: q.id,
      chosen: selected[i],
      correct: selected[i] === q.answer,
    }));
    onFinish(answers);
    ...
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "Enter") {
        if (isExam) {
          ...
          if (!revealed && selected[idx] !== null) setRevealed(true);
  ...
  function handleSelect(i: number) {
    if (revealed) return;
    setSelected((prev) => { const a = [...prev]; a[idx] = i; return a; });
  }
    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-6">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <p className="mb-6 text-base font-semibold leading-relaxed text-gray-900 md:text-lg">
            {q.question}
          </p>
          ...
          {revealed && mode === "entrainement" && (
            <div className={`mt-5 rounded-xl border p-4 text-sm leading-relaxed ${selected[idx] === q.answer ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-900"}`}>
              <p className="mb-1.5 font-bold">
                {selected[idx] === q.answer ? "✓ Bonne réponse !" : "✗ Mauvaise réponse — voici l'explication :"}
              </p>
              <p>{q.explanation}</p>
            </div>
          )}
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div className="flex flex-1 justify-end gap-2">
            ...
            {!isExam && !revealed && (
              <button onClick={() => setRevealed(true)} disabled={selected[idx] === null}
                ...
                {wrongAnswers.map(({ answer: a, question: q }, wi) => {
                  ...
                  return (
                    <div key={wi} className="p-5">
                      ...
                      <p className="mb-3 text-sm font-semibold text-gray-900">{q.question}</p>
                      <div className="space-y-1.5">
                        {q.options.map((opt, i) => {
                          const isRight = i === q.answer;
                          const isChosen = i === a.chosen;
...
export default function ISTQBTraining() {
  ...
  const [examTitle, setExamTitle]   = useState("ISTQB FL");
