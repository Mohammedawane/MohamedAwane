...
export default function ISTQBCheckoutButton({
  lang,
  isFr,
  course = "istqb-fondation",
  priceLabel = "24 €",
}: {
  lang: string;
  isFr: boolean;
  ...
  async function handleClick() {
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ course, lang }),
      });
      ...
      >
        {loading ? (
          ...
          <>
            ...
            {isFr ? `Commencer la préparation — ${priceLabel}` : `Start preparing — ${priceLabel}`}
          </>
        )}
      </button>
