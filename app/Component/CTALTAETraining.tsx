...
import {
  ...
  type CTALTAEQuestion as Question, type CTALTAEDomain as Domain, type Difficulty, type ExamInfo,
} from "@/app/data/istqb-ctal-tae-questions";
...
const LS_KEY = "ctaltae_v1_progress";
...
function HomeScreen({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-gradient-to-br from-indigo-900 via-blue-800 to-slate-900 px-6 py-10 text-white">
      ...
      <div className="mx-auto max-w-3xl px-5 py-8 space-y-8">
        ...
        <section>
          ...
          <button
            ...
            className="group w-full rounded-2xl border-2 border-dashed border-gray-300 bg-white p-6 text-left transition-all hover:border-indigo-400 hover:bg-indigo-50"
          >
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700 transition-colors group-hover:bg-indigo-200">
        ...
        <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-sm text-indigo-700">
          <strong>Rappel CTAL-TAE :</strong> 40 questions · 60 minutes · Seuil de réussite : 65% · certaines questions demandent de sélectionner plusieurs réponses
        </div>
      </div>
    </div>
  );
...
function ExamCard({ exam, result, passed, onStart }: { exam: ExamInfo; result: ExamResult | null; passed: boolean; onStart: () => void }) {
  ...
  const numBg = !result ? "bg-indigo-700 text-white" : passed ? "bg-emerald-500 text-white" : "bg-amber-500 text-white";
        ...
        <div className="space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              {([
                ...
                <button key={m.val} onClick={() => setMode(m.val)}
                  className={`rounded-xl border-2 p-4 text-left transition-all ${mode === m.val ? "border-indigo-600 bg-indigo-50" : "border-gray-200 hover:border-gray-300"}`}>
                  <p className={`font-semibold text-sm ${mode === m.val ? "text-indigo-700" : "text-gray-800"}`}>{m.label}</p>
            ...
            <div className="flex flex-wrap gap-2">
              {(["tous", "facile", "moyen", "difficile"] as const).map((d) => (
                <button key={d} onClick={() => setDiff(d)}
                  className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all ${diff === d ? "bg-indigo-700 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
          ...
          <div>
            <p className="mb-3 text-sm font-semibold text-gray-700">
              Nombre de questions : <span className="text-indigo-700 font-bold">{Math.min(count, available)}</span>
            </p>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-700 p-4 font-bold text-white shadow transition-all hover:-translate-y-0.5 hover:bg-indigo-800 disabled:cursor-not-allowed disabled:opacity-50"
          >
              ...
              return (
                <button key={i} onClick={() => handleSelect(i)} disabled={revealed} className={cls}>
                  <span className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg text-xs font-bold ${
                    ...
                    isSel ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-500"
                  }`}>
                    {LETTERS[i]}
                  </span>
          ...
          <div className="flex flex-1 justify-end gap-2">
            ...
            {!isExam && !revealed && (
              <button onClick={() => setRevealed(true)} disabled={!canValidate}
                className="rounded-xl bg-indigo-700 px-5 py-2 text-sm font-bold text-white hover:bg-indigo-800 disabled:opacity-40">
                Valider
              </button>
            )}
            {!isExam && revealed && (
                ...
                return (
                  <button
                    ...
                    className={`h-7 w-7 rounded-lg text-xs font-bold transition-all ${
                      isCurrent  ? "bg-indigo-700 text-white ring-2 ring-indigo-300 ring-offset-1" :
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-10 border-b border-gray-200 bg-white px-4 py-3 shadow-sm">
        <div className="mx-auto flex max-w-2xl items-center justify-between">
          ...
          <button onClick={onRetry} className="text-sm font-semibold text-indigo-700 hover:text-indigo-800">Recommencer</button>
        </div>
      </div>

      <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
        {/* Score hero */}
        <div className={`rounded-2xl p-8 text-center shadow-lg ${passed ? "bg-gradient-to-br from-emerald-600 to-teal-700" : "bg-gradient-to-br from-slate-800 to-indigo-900"} text-white`}>
                ...
                {wrongAnswers.map(({ answer: a, question: q }, wi) => {
                  ...
                  return (
                    <div key={wi} className="p-5">
                      ...
                      <div className="mt-3 rounded-lg bg-indigo-50 p-3 text-xs text-indigo-800 leading-relaxed border border-indigo-100">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={onRetry}
            className="rounded-xl border-2 border-indigo-700 py-3 text-sm font-bold text-indigo-700 transition hover:bg-indigo-50">
            Recommencer
          </button>
          <button onClick={onHome}
            className="rounded-xl bg-indigo-700 py-3 text-sm font-bold text-white transition hover:bg-indigo-800">
            Accueil →
          </button>
        </div>
      </div>
...
export default function CTALTAETraining() {
  ...
  const [examTitle, setExamTitle]   = useState("ISTQB CTAL-TAE");
